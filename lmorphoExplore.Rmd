...
library(nlmd)
```
This document is for exploratory analysis and validation of the NLMD as I get it ready for publication.  Mostly for documentaiton and my own purposes.
#Step 1: Build SQLite database from source csv files
```{r makeData}
if(!file.exists("lakemorpho.sqlite3")){
  source("buildlmdb.R")
...
## Find problems 
There are some duplicate COMIDS.  May be some other issues, still need to dig
into it.
```{r findProbs}
# Find dups
nlmdata_dups<-lmorphoAll_sqlite$>$
  group_by(COMID)$>$
  summarise(count=n())$>$
  filter(count>1)$>$
  collect(-1)
# Grab morpho data for each duplicate
# and check.  If identifcal (or close to it as some of the length stuff 
# based on a sample)
filter(lmorphoAll_sqlite,COMID%in%nlmdata_dups$COMID)%>%
  select(COMID,region,SurfaceArea)%>%
  arrange(COMID)
...
## Select NLA Lakes
```{r getNLA}
#Replace input with cleaned up data (i.e. no dups)
